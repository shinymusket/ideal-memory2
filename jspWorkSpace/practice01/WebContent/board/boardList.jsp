	...
	<div>
		...
		<div class="w3-main w3-margin-bottom" style="margin-left: 340px; margin-right: 40px; width: 60%;">
			...
			<div class="w3-pannel w3-bar">
				<%-- 검색창 --%>
				<form action="/practice01/BoardList.do" method="get" class="w3-bar-item">
					<div>
				...
				<c:if test="${ fcount > 0 && sel != null && find != null}">
					<c:forEach var="li" items="${ flist }">
						<div class="w3-container w3-border w3-hover-border-red" style="margin: 0px">
							...
							<c:choose>
								...
								<c:otherwise>
									<a href="/practice01/BoardList.do?section=${ section }&num=${ i }" class="w3-bar-item w3-button w3-small">${ i }</a>
								</c:otherwise>
							</c:choose>
					</c:forEach>
						<c:if test="${ pageCount > endPage }">
							<a href="/practice01/BoardList.rcd?section=${ section }&num=${ startPage + 10 }" class="w3-bar-item w3-button w3-small">다음</a>
						</c:if>
					</c:if>
					...
					<c:if test="${ pageCount !=0 && sel != null && find != null }">
						<c:if test="${ startPage > 10 }">
							<a href="/practice01/BoardList.do?section=${ section }&num=${ startPage - 10 }&sel=${ sel }&find=${ find }" class="w3-bar-item w3-button w3-small">이전</a>
						</c:if>
						<c:forEach var="i" begin="${ startPage }" end="${ endPage }">
							<c:choose>
								...
								<c:otherwise>
									<a href="/practice01/BoardList.do?section=${ section }&num=${ i }&sel=${ sel }&find=${ find }" class="w3-bar-item w3-button w3-small">${ i }</a>
								</c:otherwise>
							</c:choose>
						</c:forEach>
						<c:if test="${ pageCount > endPage }">
							<a href="/practice01/BoardList.do?section=${ section }&num=${ startPage + 10 }&sel=${ sel }&find=${ find }" class="w3-bar-item w3-button w3-small">다음</a>
						</c:if>
					</c:if>
				</div>
			</div>
